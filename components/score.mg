...
fn setup(drawer: graphic::Drawer, window: js::Window): Component {
  ...
  let image = bitmap::load_image(digits_bmp);
...
fn draw(c: Component, s: state::State) {
  ...
  defer mem::dealloc_array::<i32>(digits);
  let n_digits = 0;
  if score == 0 {
    digits[n_digits].* = 0;
    n_digits += 1;
